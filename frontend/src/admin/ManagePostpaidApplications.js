...
function ManagePostpaid() {
  ...
  const [showForm, setShowForm] = useState(false); // State để điều khiển việc hiển thị form
  ...
  return (
    <div className="manage-postpaid postpaid-container">
      ...
      <table className="postpaid-table">
        ...
        <tbody>
          {partners.map((partner) => (
            <tr key={partner.id}>
              ...
              <td>
                <button className="addButton" onClick={() => setShowForm(true)}>Thêm</button>
                <button className="actionButton" onClick={() => editPartner(partner)}>Sửa</button>
                <button className="addButton" onClick={() => deletePartner(partner.id)}>Xóa</button>
              </td>
            </tr>
          ))}
        </tbody>
        ...
        <div className="partner-form">
          ...
          {/* Nút huỷ */}
          <button onClick={() => setShowForm(false)} style={{ marginLeft: '10px' }}>Huỷ</button>
        </div>
      )}
        ...
        <tbody>
          {applications.map((app) => (
            <tr key={app.id} style={{ opacity: app.status === 'rejected' ? 0.5 : 1 }}>
              ...
              <td>{new Intl.NumberFormat('vi-VN').format(app.monthly_income)} VND</td>
